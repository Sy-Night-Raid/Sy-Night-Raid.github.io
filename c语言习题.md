...
* 初始化**数组(array)**：int powers [8] = {1,2,4,6,8,16,32,64}; //上面的初始化就是把powers[0]=1,powers[1]=2,以此类推……                                      ```
写一个小程序：打印每个月的天数
#include<stdio.h>
#define MONTH 12      //其实直接用12就很方便，为什么要定义一个MONTH呢，这是程序员推荐的做法。例如，如果我们现在要记录11个月的数据，只需要把#define后面的12就行，不用在程序的其他地方查找所有使用过12的地方去修改
// const声明和define有些类似，const int MONTH = 12；表面上和define的作用是一样的，但是不同的点是const声明是只读的，就是一旦声明为const就不能改变、不能再给它赋值。
```
int main()
{
	int year;
...
```
* 指定初始化数组：对于一般的初始化，在初始化一个元素后，未初始化的元素都会被设置为0.     
int arr[6] = {0,0,0,0,0,212}是我们上面学的用法，还可以int arr[6] = {[5]=212} //直接将arr[5]初始化为212
* 多维数组：气象研究员Candy为完成她的研究项目要分析5年内每个月的降水量数据，她首先要解决的问题是如何表示数据。一个方案是创建60个变量，每个变量存储一个数据项；我们现在知道使用一个内含60个元素的数组比创建60个变量好，但是如果能把各年的数据分开存储会更好，即创建5个数组，每个数组有12个元素。但其实这样做也不最简便，因为如果我要研究50年的降水量，岂不是要创建50个数组。
	1. 处理这种情况应该用数组的数组。**主数组（master array）** 有5个元素（每个元素表示一年），每个元素是内含12个元素的数组（每个元素表示一个月）
	2. `float rain[5][12]; //内含5个数组元素的数组，每个数组元素内含12个float类型的元素(这就是二维数组)`
	3. 初始化二维数组：
```
		const float rain[5][12] = 
	{
		{4.3,1.2,2.3,1.6,5.6,7.1,3.5,4.1,2.1,2.6,5.5,4.9},
		{12个数}, 
		{12个数}, 
		{12个数}, 
		{12个数}
	};   //这个初始化使用了5个数值列表，每个数值列表都用花括号括起来。第1个列表的数据用于初始化数组的第1行，第2个列表的数据用于初始化数组的第2行，以此类推。注意匹配问题，如果第1个列表中只有10个数字，那么它会按顺序初始化 即只会初始化数组第1行的前10个元素，而最后两个元素将被默认初始化为0.
```
```
getsputs.c
在读取字符串时，scanf()和转换说明%s只能读取一个单词。但在程序中我们经常要输入一段句子不仅仅是一个单词。这时就要用到gets()函数，它读取整行输入，直到遇到换行符，然后丢弃换行符，存储其余字符，并在这些字符的末尾添加一个空字符使其成为一个C字符串。它经常和puts()函数配对使用；puts()函数用于显示字符串，并在末尾添加换行符。
#include<stdio.h>
int main()
{
	char words[80];
